...
final_NGTs<-readRDS(file="final_NGTs.rds")
...
```{r}
#catalogue of all mutations so we can determine how many patients were mutated for each gene and how many different mutations were seen in total for each gene.
...
Now we are going to make the oncoprint plot that is in Supplementary Figure 1a. 
...
# identify whether a variant is an indel, Nonsense or Missense mutation
# group samples together and pivot the matrix into wide format
...
```
Supplementary Figure 1c : the total number of mutations identified per gene

```{r}
# Set the levels of the Gene column from most to least prevalent for plotting purposes
# Genes are ranked by the number of identified protein coding mutations from highest to lowest. Genes with zero identified mutations are not listed.
library(ggplot2)
...
Supplementary Figure 1d : Total number of patients mutated for each gene
...
```
Supplementary Figure 1e : Number of mutated genes per patient
...
gg_mutated_genes_per_patient
#ggsave("SFig1e.pdf",width=6,height=5)
```

Supplementary Figure 1f : Total number of mutations per patient
```{r}
...
gg_mutations_per_patient<- final_mut_melt%>%
                                        ...
                                        scale_y_continuous(expand=c(0,0))+
                                        scale_x_continuous(expand=c(0,0))
gg_mutations_per_patient
#ggsave("SFig1f.pdf",width=6,height=5)
```

Put these four figurs together : 
...
Build a data frame indicating the mutation status of various patients with regard to epigenetic modifiers (DNMT3A,TET2, ASXL1, IDH1/2, DTAI) and signalling genes (FLT3, vs JAK2 vs NRAS/KRAS)
...
Next, calculate a few metrics for each patient sample.
...
Next, merge the data frames together and plot Figure 1a.

Figure 1a : Number of identified mutations in each sample (mean ± s.e.m.) with samples by cohort.
...
```{r}
#Compute statistics for the different group comparisons.
...
Top, number of cells (mean ± 95% confidence intervals (CI)) identified with a given genotype and ranked by decreasing frequency. 

Bottom, heatmap indicates mutation zygosity for each clone.
...
#dev.off()
```
Figure 1c : Number of unique clones per sample for each cohort
...
```
Figure 1d : clonal diversity calculated by the Shannon diversity index.
...
pvalues_Shannon
```
Figure 1e : fraction of cells in the dominant clone

dominant clone : were defined as the largest mutant clone in the sample
